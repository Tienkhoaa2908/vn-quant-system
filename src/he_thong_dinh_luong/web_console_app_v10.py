"""VN Quant Local Terminal v10: one-share periodic contribution planning."""
...
from .contribution_portfolio_v18 import (
...
def build_app(
    ...
    ui.add_css("""
        ...
        .v10-contribution-button { position:fixed; right:24px; bottom:24px; z-index:700; }
        .v10-contribution-dialog { width:min(1320px,96vw) !important; max-width:1320px !important; }
    """)
    refs: dict[str, Any] = {}

    with ui.dialog() as dialog, ui.card().classes("v10-contribution-dialog p-4"):
        with ui.row().classes("w-full items-center justify-between gap-3"):
            with ui.column().classes("gap-0"):
                ui.label("Góp vốn định kỳ · chia theo từng cổ phiếu").classes(
                    "text-h6"
                )
                ui.label(
        ...
        ui.label(
            "Bước số lượng là 1 cổ phiếu. Phần 1–99 cổ phiếu phải đặt lệnh LO trên bảng lô lẻ; giá thực tế có thể khác bảng lô chẵn."
    ...
    def run_plan() -> None:
        ...
        try:
            ...
            run, paths, _, _, _, current_predictions, _ = terminal_base._latest_bundle(
                config
            )
            price_rows = terminal_base._csv(paths["publication"], limit=10_000_000)
            prices, price_day = latest_price_map(price_rows)
            sector_map = {
                str(row.get("symbol") or row.get("ma") or "")
                .strip()
                .upper(): str(row.get("sector") or row.get("industry") or "").strip()
            ...
            rows = [
                {
                    ...
                    "round_quantity": raw.get(
                        "recommended_round_lot_quantity"
                    ),
                    "odd_quantity": raw.get("recommended_odd_lot_quantity"),
            ...
            refs["summary"].set_text(
                ...
                f"{plan['contribution_status']} · thiếu tới 1 cổ phiếu hợp lệ kế tiếp "
                f"{_vnd(plan['cash_shortfall_to_next_share_vnd'])}."
            )
            warning_parts = list(plan.get("warnings", []))
            warning_parts.extend(plan.get("hard_risk_breaches", []))
            refs["warning"].set_text(
                " · ".join(str(item) for item in warning_parts)
            )
            ui.notify(
            ...
            ui.notify(
                "Không tạo được kế hoạch góp vốn.",
                type="negative",
                timeout=9000,
            )
        finally:
    ...
    ).classes("v10-contribution-button"):
        pass
