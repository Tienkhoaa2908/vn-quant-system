"""VN Quant Local Terminal v7 with integrated Model Lab workflow."""
...
def build_app(ui: Any, config: LocalWebConfig, jobs: JobStore, portfolio: PortfolioStore) -> None:
    ...
    ui.add_css("""
        .v7-command { position:fixed; left:18px; right:18px; top:68px; z-index:500;
        ...
        .v7-stage { min-width:190px; }
        .v7-model-drawer { width:min(1120px,96vw) !important; }
        .terminal-shell { padding-top:128px !important; }
    """)
    refs: dict[str, Any] = {}
    state: dict[str, object] = {"workflow": None, "handled": set()}

    with ui.card().classes("v7-command p-3"):
        with ui.row().classes("w-full items-center justify-between gap-3 flex-wrap"):
            with ui.column().classes("gap-0"):
                ui.label("Workflow thống nhất").classes("text-subtitle1 text-weight-bold")
                refs["summary"] = ui.label(
                    "Một nút chạy dữ liệu, dự đoán hiện tại, Model Lab OOS và paper."
        ...
        with ui.row().classes("w-full gap-4 flex-wrap mt-2"):
            refs["market_stage"] = ui.label("Dữ liệu: chờ").classes("v7-stage text-caption")
            refs["model_stage"] = ui.label("Model Lab: chờ").classes("v7-stage text-caption")
    ...
    with ui.dialog() as dialog, ui.card().classes("v7-model-drawer p-4"):
        with ui.row().classes("w-full items-center justify-between"):
            ui.label("Model Lab — leaderboard OOS").classes("text-h6")
        ...
        refs["table"] = ui.table(
            columns=[
                ...
                {"name": "gate", "label": "Gate", "field": "gate", "sortable": True},
    ...
    def create_model_job(*, force: bool) -> str:
        ...
        command = [
            ...
            "--input-zip", str(config.data_root / "prediction_input.zip"),
        ...
        job_id = jobs.create_job(
            ...
            output_dir=output,
            log_path=log_path,
            parameters={"force": force, "integrated": True},
        )
    ...
    async def start() -> None:
        ...
        ui.notify("Workflow đã bắt đầu; kết quả tự cập nhật trên trang chính.", type="positive")
        ...
        for job in reversed(recent):
            ...
            if job.get("kind") == "terminal_refresh" and job_id == workflow.get("market_job"):
                ...
                if job.get("status") == "SUCCESS" and workflow.get("run_models"):
                    try:
                        model_job = create_model_job(force=bool(workflow.get("force")))
                        updated = dict(workflow)
                        updated["model_job"] = model_job
                        state["workflow"] = updated
                        refs["model_stage"].set_text("Model Lab: đang chạy")
                        return
                    except Exception as exc:
                        refs["model_stage"].set_text(f"Model Lab: lỗi khởi chạy {type(exc).__name__}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    def health() -> dict[str, object]:
        return {
            ...
            "output_created_before_model_execution": True,
