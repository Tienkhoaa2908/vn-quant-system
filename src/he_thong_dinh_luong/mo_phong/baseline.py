...
def baseline_mua_va_giu(
    du_lieu_gia: Iterable[thanh_gia],
    *,
    ngay_tin_hieu: date | None = None,
    cac_ma: Iterable[str] | None = None,
) -> list[ty_trong_muc_tieu]:
    ...
    du_dieu_kien = [
        muc.ma
        for muc in theo_ngay[ngay]
        if (tap_ma is None or muc.ma in tap_ma)
        and muc.thuoc_tap_co_phieu is not False
        and muc.dat_thanh_khoan is not False
    ]
    return _chia_deu(ngay, du_dieu_kien, "mua_va_giu")


def baseline_can_bang_deu(
    du_lieu_gia: Iterable[thanh_gia],
    *,
    cac_ngay_tai_can_bang: Iterable[date],
) -> list[ty_trong_muc_tieu]:
    theo_ngay = _theo_ngay(du_lieu_gia)
    ...
    for ngay in sorted(set(cac_ngay_tai_can_bang)):
        ...
        cac_ma = [
            muc.ma
            for muc in theo_ngay[ngay]
            if muc.thuoc_tap_co_phieu is True and muc.dat_thanh_khoan is True
        ]
        ket_qua.extend(_chia_deu(ngay, cac_ma, "can_bang_deu"))
    return ket_qua


def baseline_ma250_dong_luong(
    du_lieu_gia: Iterable[thanh_gia],
    *,
    cac_ngay_tai_can_bang: Iterable[date],
    top_k: int,
    ...
    for ngay in sorted(set(cac_ngay_tai_can_bang)):
        ...
        ung_vien = [
            muc
            for muc in theo_ngay[ngay]
            if muc.thuoc_tap_co_phieu is True
            and muc.dat_thanh_khoan is True
            and muc.tren_ma250 is True
            and muc.dong_luong is not None
        ]
        ung_vien.sort(key=lambda muc: (-muc.dong_luong, muc.ma))
        cac_ma = [muc.ma for muc in ung_vien[:top_k]]
        ket_qua.extend(_chia_deu(ngay, cac_ma, "ma250_dong_luong"))
    return ket_qua
