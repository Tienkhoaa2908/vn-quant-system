...
## Trạng thái hậu gộp PR #23 và đóng cửa chạy lại kỹ thuật Mốc 4 trên tập rộng

- `PR23_MERGED`: PR #23 đã gộp bằng merge commit `3ff2ed3c8b18eebd9095e9af182b004f4d0aee67`; head đã gộp là `f813cd3cda4c0129f53deb9ffb8a9a42f18b5220`.
- `POST_MERGE_MAIN_CI_PASSED`: workflow `kiem_tra_tu_dong`, Run ID `30436034585`, trên `main` đã `completed/success`.
- Ubuntu Job `90523874732` và Windows Job `90523875099` đều thành công.
- `MOC4_BROAD_TECHNICAL_RERUN_COMPLETE`: lần chạy kỹ thuật rộng dùng publication nguồn gồm `121` mã và `231151` dòng.
- `120` mã có dự báo/xếp hạng; `TMS` được loại có kiểm soát vì không đủ lịch sử cho các đặc trưng bắt buộc, chủ yếu MA250, MA120 và các cửa sổ liên quan.
- Tổng `36` fold, `34` fold thành công. `fold_035` và `fold_036` có `test_rong` tại biên cuối dữ liệu do không còn đủ `20` phiên tương lai để tạo nhãn; đây không phải lỗi huấn luyện.
- Có `3840` dự báo test Logistic và `3840` dự báo test đường cơ sở động lượng.
- Publication gồm đúng `23` tệp sản phẩm.
- Hai lượt kiểm toán độc lập đều đạt; các sản phẩm kiểm toán tương ứng giống nhau tuyệt đối theo byte và SHA-256.
- Hash đầu vào khớp trước và sau chạy; không có bằng chứng đầu vào bị thay đổi.
- Logistic chỉ là đường chuẩn kỹ thuật và yếu hơn đường cơ sở động lượng trong lần chạy này.
- Không được diễn giải kết quả thành alpha, tín hiệu vận hành, khả năng giao dịch thực hoặc khuyến nghị đầu tư.
- `RESEARCH_GATE=FAIL`, mã bắt buộc `PRICE_BASIS_UNCONFIRMED`.
- Các điểm chặn nghiên cứu giữ nguyên: `VN100_POINT_IN_TIME_HISTORY_INCOMPLETE`, `HOSE_EOD_CROSSCHECK_INCOMPLETE`, `CORPORATE_ACTION_INVENTORY_INCOMPLETE`, `PRICE_BASIS_UNCONFIRMED`.
- `MOC5_IMPLEMENTATION_PAUSED`: Mốc 5 tiếp tục tạm dừng; PR #20 không bị sửa và không có mã Mốc 5 được triển khai.
