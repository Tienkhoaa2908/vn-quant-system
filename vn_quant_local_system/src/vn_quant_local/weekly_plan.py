...
P1 canonical vẫn dùng Top-10 và buffer Top-20 hai tháng. V44.2 bổ sung chế độ
nghiên cứu mua tối đa nhiều mã trong cùng ngân sách tuần. Không gửi lệnh broker.
"""
...
PLANNER_VARIANT = "V44_2_MULTI_BUY_WITHIN_WEEKLY_BUDGET"
...
def allocate_buy_orders(candidates: Sequence[Mapping[str, object]], *, budget_vnd: float, max_orders: int, cost_bps: float) -> list[dict[str, object]]:
    if budget_vnd <= 0.0:
        return []
    ...
    for raw in candidates:
        ...
        ordered.append(row)
    ordered.sort(key=lambda row: (-float(row["priority"]), -float(row.get("target_gap_vnd") or 0.0), int(row.get("rank") or 10**9)))
    ...
    while selected:
        ...
        best_residual = 0.0
        for row in selected:
            residual = float(row["budget_ceiling_vnd"]) - float(row["estimated_cost_vnd"])
            one_share = float(row["one_share_cost_vnd"])
        ...
        best["estimated_cost_vnd"] = float(best["estimated_cost_vnd"]) + extra_cost
...
def _latest_two_monthly_rankings() -> list[dict[str, object]]:
    with state_db() as db:
        signal_days = [str(row[0]) for row in db.execute("SELECT DISTINCT signal_day FROM rankings WHERE signal_kind='MONTHLY_CANONICAL' ORDER BY signal_day DESC LIMIT 2").fetchall()]
        result: list[dict[str, object]] = []
        for signal_day in signal_days:
            run = db.execute("SELECT r.run_id,r.finished_at FROM runs r JOIN rankings k ON k.run_id=r.run_id WHERE r.status='SUCCESS' AND k.signal_kind='MONTHLY_CANONICAL' AND k.signal_day=? ORDER BY r.finished_at DESC LIMIT 1", (signal_day,)).fetchone()
            if run is None:
                continue
            rows = [dict(row) for row in db.execute("SELECT * FROM rankings WHERE run_id=? AND signal_kind='MONTHLY_CANONICAL' ORDER BY rank", (run["run_id"],)).fetchall()]
...
def _latest_market_prices(symbols: Sequence[str]) -> tuple[str, dict[str, float]]:
    ...
    try:
        day = str(db.execute("SELECT MAX(day) FROM bars WHERE upper(asset_type)='INDEX'").fetchone()[0])
        prices: dict[str, float] = {}
        multiplier = float(load_config().get("model", {}).get("price_multiplier", 1000.0))
        for symbol in sorted(set(symbols)):
            row = db.execute("SELECT close FROM bars WHERE upper(asset_type)='STOCK' AND symbol=? AND day<=? ORDER BY day DESC LIMIT 1", (symbol, day)).fetchone()
            if row is not None:
...
def _position_reviews(*, holdings: Mapping[str, int], sellable: Mapping[str, int], prices: Mapping[str, float], current_rank: Mapping[str, int], previous_rank: Mapping[str, int], target_weights: Mapping[str, float], account_value: float, have_two_months: bool) -> list[dict[str, object]]:
    ...
    for symbol in sorted(holdings):
        ...
        if have_two_months and rank_now > 20 and rank_prev > 20:
            action = "EXIT_CANDIDATE" if int(sellable.get(symbol, quantity)) > 0 else "WAIT_SELLABLE"
        ...
        if action in {"HOLD", "HOLD_NO_ADD"} and target_weight > 0.0 and actual_weight > max(0.20, target_weight * 1.50):
            action = "REVIEW_TRIM"
...
def create_weekly_plan(*, weekly_budget_vnd: float | None = None, maximum_buy_orders: int | None = None) -> dict[str, object]:
    ...
    previous_rank = {str(row["symbol"]): int(row["rank"]) for row in rankings[1]["rows"]} if len(rankings) > 1 else {}

    account = account_snapshot()
    holdings = {str(row["symbol"]): int(row["quantity"]) for row in account["holdings"] if int(row["quantity"]) > 0}
    account_row = account["account"]
    cash = float(account_row["cash_vnd"])
    configured_budget = float(account_row["weekly_contribution_vnd"])
    requested_budget = configured_budget if weekly_budget_vnd is None else float(weekly_budget_vnd)
    if requested_budget <= 0.0:
        raise ValueError("Ngân sách tuần phải lớn hơn 0")
    ...
    max_orders = default_max_orders if maximum_buy_orders is None else int(maximum_buy_orders)
    if max_orders < 1 or max_orders > 5:
        raise ValueError("Số lệnh mua tối đa phải nằm trong 1..5")
    budget = min(cash, requested_budget)

    broker = latest_broker_portfolio()
    broker_positions = {str(row["symbol"]): row for row in (broker or {}).get("positions", [])}
    sellable = {symbol: int(row.get("sellable_quantity") or holdings.get(symbol, 0)) for symbol, row in broker_positions.items()}

    symbols = set(holdings) | {str(row["symbol"]) for row in current_rows[:20]}
    market_day, prices = _latest_market_prices(sorted(symbols))
    holdings_value = sum(quantity * prices.get(symbol, 0.0) for symbol, quantity in holdings.items())
    account_value = cash + holdings_value

    target_rows = current_rows[:10]
    weights = capped_inverse_vol_weights(target_rows, cap=0.15)
    established = sum(1 for row in target_rows if holdings.get(str(row["symbol"]), 0) > 0)
    ...
    for row in target_rows:
        ...
        target_value = target_weight * account_value
        ...
        candidates.append({
            ...
            "underweight_pct": max(target_weight - actual_weight, 0.0),
            "effective_cap": effective_cap,
            "budget_ceiling_vnd": budget_ceiling,
            "volatility_60": float(row.get("volatility_60") or 0.0),
            "low_volatility_pct": float(row.get("low_volatility_pct") or 0.0),
            "relative_strength_120_pct": float(row.get("relative_strength_120_pct") or 0.0),
            "high_52_week_pct": float(row.get("high_52_week_pct") or 0.0),
        })
    candidates.sort(key=lambda row: (-float(row["underweight_pct"]), -float(row["target_gap_vnd"]), int(row["rank"])))

    buy_orders = allocate_buy_orders(candidates, budget_vnd=budget, max_orders=max_orders, cost_bps=planning_cost_bps)
    single_order = allocate_buy_orders(candidates, budget_vnd=budget, max_orders=1, cost_bps=planning_cost_bps)
    total_buy = sum(float(row["estimated_cost_vnd"]) for row in buy_orders)
    ...
    exit_candidates = [row for row in reviews if row["action"] == "EXIT_CANDIDATE"]
    ...
    rationale = {
        ...
        "portfolio_source": "DNSE_BROKER_SNAPSHOT" if broker else "MANUAL_LOCAL_ACCOUNT",
        "broker_snapshot_id": (broker or {}).get("snapshot_id"),
        "broker_snapshot_time": (broker or {}).get("captured_at"),
        "broker_safe_cash_vnd": cash,
        "requested_weekly_budget_vnd": requested_budget,
        "spendable_budget_vnd": budget,
        ...
        "buffer_rule": "EXIT_CANDIDATE_ONLY_IF_OUTSIDE_TOP20_TWO_CONSECUTIVE_MONTHLY_SIGNALS",
        "sell_proceeds_reused_only_after_actual_broker_sync": True,
        "odd_lot_share_unit": 1,
        "multi_buy_research_status": "IMPLEMENTED_NOT_YET_HISTORICALLY_REVALIDATED",
    }
    ...
    plan = {
        ...
        "weekly_budget_vnd": requested_budget,
        "spendable_budget_vnd": budget,
        "remaining_budget_vnd": max(budget - total_buy, 0.0),
        "buy_orders": buy_orders,
        "single_order_baseline": single_order,
        "position_reviews": reviews,
    ...
    with state_db() as db:
        db.execute("INSERT INTO weekly_plans(plan_id,created_at,ranking_run_id,contribution_vnd,available_cash_vnd,buy_symbol,buy_quantity,estimated_buy_value_vnd,sell_symbols_json,rationale_json,research_only) VALUES(?,?,?,?,?,?,?,?,?,?,1)", (
            plan_id, plan["created_at"], latest["run_id"], requested_budget, cash, plan["buy_symbol"], plan["buy_quantity"], total_buy,
            json.dumps([row["symbol"] for row in exit_candidates]), json.dumps(rationale, ensure_ascii=False, sort_keys=True),
        ))
...
def latest_weekly_plan() -> dict[str, object] | None:
    ...
    result["single_order_baseline"] = result["rationale"].get("single_order_baseline", [])
    result["position_reviews"] = result["rationale"].get("position_reviews", [])
    result["exit_candidates"] = [row for row in result["position_reviews"] if row.get("action") == "EXIT_CANDIDATE"]
    result["weekly_budget_vnd"] = result["contribution_vnd"]
    result["spendable_budget_vnd"] = result["rationale"].get("spendable_budget_vnd", 0.0)
    result["remaining_budget_vnd"] = max(float(result["spendable_budget_vnd"]) - sum(float(item.get("estimated_cost_vnd") or 0.0) for item in result["buy_orders"]), 0.0)
    return result
