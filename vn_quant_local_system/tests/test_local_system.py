...
class LocalSystemTests(unittest.TestCase):
    ...
    def test_multi_buy_diversifies_when_budget_allows(self) -> None:
        candidates = [
            {"symbol": "AAA", "rank": 1, "price_vnd": 50_000, "budget_ceiling_vnd": 200_000, "underweight_pct": 0.12, "target_gap_vnd": 200_000},
            {"symbol": "BBB", "rank": 2, "price_vnd": 60_000, "budget_ceiling_vnd": 180_000, "underweight_pct": 0.10, "target_gap_vnd": 180_000},
            {"symbol": "CCC", "rank": 3, "price_vnd": 70_000, "budget_ceiling_vnd": 140_000, "underweight_pct": 0.08, "target_gap_vnd": 140_000},
        ]
        orders = allocate_buy_orders(candidates, budget_vnd=200_000, max_orders=3, cost_bps=0.0)
        self.assertEqual([row["symbol"] for row in orders], ["AAA", "BBB", "CCC"])
        self.assertLessEqual(sum(row["estimated_cost_vnd"] for row in orders), 200_000)

    def test_single_order_baseline_stays_single(self) -> None:
        candidates = [
            {"symbol": symbol, "rank": index, "price_vnd": 50_000, "budget_ceiling_vnd": 300_000, "underweight_pct": 0.20 - index / 100, "target_gap_vnd": 300_000}
            for index, symbol in enumerate(("AAA", "BBB", "CCC"), start=1)
        ]
        orders = allocate_buy_orders(candidates, budget_vnd=250_000, max_orders=1, cost_bps=50.0)
        self.assertEqual(len(orders), 1)
        self.assertLessEqual(sum(row["estimated_cost_vnd"] for row in orders), 250_000)
