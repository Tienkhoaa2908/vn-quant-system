...
class TurnoverPolicyStabilityV33Tests(unittest.TestCase):
    ...
    def test_posthoc_non_three_cap_cannot_be_freeze_candidate(self) -> None:
        summary = [
            {
                "fixed_replacement_cap": 3,
                "base_relative_total_return": 0.05,
                "stress_relative_total_return": 0.04,
                "base_leave_best_period_out_relative_total_return": -0.01,
                "base_mean_turnover": 0.30,
            },
            {
        ...
        paired = [
            {
                "fixed_replacement_cap": 3,
                "bootstrap_probability_delta_positive": 0.60,
                "leave_best_3_mean_net_excess_delta": -0.001,
            },
            {
                "fixed_replacement_cap": 6,
        ...
        decisions, recommendation = v33._decision_rows(summary, paired)
        by_cap = {
            int(row["fixed_replacement_cap"]): row for row in decisions
        }
        self.assertTrue(by_cap[6]["sensitivity_gate_passed"])
        self.assertFalse(by_cap[6]["future_holdout_freeze_candidate"])
        self.assertFalse(by_cap[3]["future_holdout_freeze_candidate"])
        self.assertEqual(
            recommendation,
