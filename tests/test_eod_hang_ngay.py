...
class _FakeSource:
    ...
    def fetch(self, symbol: str, start: date, end: date, *, is_index: bool = False) -> tuple[eod.EodRow, ...]:
...
def _publication(root: Path) -> None:
    ...
    rows = [
        {
            "ma": symbol, "ngay": "2026-07-24",
            "gia_mo_cua": str(open_price), "gia_dong_cua": str(close),
            "khoi_luong": str(volume), "nguon": "vnstock_kbs",
            "phien_ban": "4.0.4", "co_so_gia": "CHUA_XAC_NHAN",
            "raw_sha256": digest * 64,
        }
        for symbol, open_price, close, volume, digest in (
            ("AAA", 10, 11, 1000, "a"),
            ("BBB", 20, 21, 2000, "b"),
        )
    ]
    ...
    for name, payload in zip(
        eod.PUB_FILES,
        (csv_payload, coverage, excluded, manifest, b"placeholder\n"),
        strict=True,
    ):
...
class TestEodHangNgay(unittest.TestCase):
    def test_entrypoint_dung_utc7(self) -> None:
        self.assertEqual(eod.VN_TZ.utcoffset(None).total_seconds(), 7 * 3600)
    ...
    def test_bat_kip_toan_bo_phien_thieu(self) -> None:
        first, second = date(2026, 7, 29), date(2026, 7, 30)
    ...
    def test_thieu_mot_phien_thi_loai_increment(self) -> None:
        first, second = date(2026, 7, 29), date(2026, 7, 30)
        accepted, reasons = eod._accepted_incremental_rows(
            symbol="AAA",
            primary_rows=(
