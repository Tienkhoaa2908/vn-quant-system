...
class AIReviewBundleV1Tests(unittest.TestCase):
    ...
    def _fixture(self, root: Path) -> dict[str, Path]:
        ...
        with zipfile.ZipFile(
            historical / "daily_prediction_input.zip",
            "w",
        ) as archive:
    ...
    def test_bundle_contains_source_evidence_prompt_and_hashes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertTrue(
                (output / "source" / "source_snapshot.zip").is_file()
            )
            self.assertTrue(
                (
                    output
            ...
            prompt = (
                output / "PROMPT_FOR_EXTERNAL_AI.md"
            ).read_text(encoding="utf-8")
            self.assertIn("selection bias", prompt)
            self.assertIn("Do not use T+1", prompt)
            report = json.loads(
                (output / "ai_review_bundle_v1.json").read_text(
                    encoding="utf-8"
                )
            )
            self.assertFalse(report["full_sqlite_store_included"])
            self.assertFalse(report["sqlite_inventory_included"])
            self.assertFalse(
                report["security_contract"]["live_capital_approved"]
            )
            manifest = json.loads(
    ...
    def test_dirty_repo_is_rejected_by_default(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            fixture = self._fixture(root)
            (fixture["repo"] / "dirty.txt").write_text(
                "dirty",
                encoding="utf-8",
            )
            with self.assertRaisesRegex(
                ValueError,
                "AI_REVIEW_DIRTY_WORKTREE",
            ):
                build_review_bundle(
                    repo_root=fixture["repo"],
                    historical_input_dir=fixture["historical"],
