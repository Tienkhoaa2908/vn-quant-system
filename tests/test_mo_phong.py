...
class KiemTraMoPhong(unittest.TestCase):
    ...
    def test_06_lam_tron_lo_va_khong_tien_am(self):
        ...
        k=chay_mo_phong([bar("A",D1),bar("A",D2,"20","20")],[tg(D1,"A","1")],cfg(kich_thuoc_lo=1));self.assertEqual(k.lenh[0].trang_thai,"da_khop");self.assertGreater(k.lenh[0].so_luong_bi_giam,0);self.assertGreaterEqual(k.so_cai[-1].tien_mat_cuoi_ngay,0)
    ...
    def test_08_thu_tu_mua_canh_tranh_tien_mat(self):
        g=[bar(m,d,mo,dong) for d,mo,dong in ((D1,"10","10"),(D2,"20","20")) for m in ("B","A")];k=chay_mo_phong(g,[tg(D1,"B","0.3"),tg(D1,"A","0.3")],cfg(kich_thuoc_lo=1));self.assertEqual([(x.ma,x.trang_thai) for x in k.lenh],[("A","da_khop"),("B","da_khop")]);self.assertGreater(k.lenh[1].so_luong_bi_giam,0)
    ...
    def test_11_co_tuc_tien_mat_ngay_thanh_toan(self):
        k=chay_mo_phong([bar("A",d) for d in (D1,D2,D3,D4)],[tg(D1,"A","0.1")],cfg(kich_thuoc_lo=1,truot_gia_bps=0,phi_mua_bps=0),[su_kien_doanh_nghiep("A","co_tuc_tien_mat",D3,D4,None,Decimal(1),"x")]);self.assertEqual([x.dong_tien_su_kien for x in k.so_cai],[0,0,0,Decimal(1000)])
    def test_12_chong_tinh_su_kien_hai_lan(self):
        with self.assertRaises(ValueError): chuan_hoa_su_kien([{"ma":"A","loai_su_kien":"co_tuc_tien_mat","ngay_hieu_luc":D1,"ngay_thanh_toan":D2,"gia_tri_tien_mat":1,"nguon":"x"}],co_so_gia="dieu_chinh")
