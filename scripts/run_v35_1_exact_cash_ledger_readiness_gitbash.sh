...
DATA_ASSURANCE_POSIX="/c/Users/welcome/Documents/vn-quant-data/reference/exact_ledger_data_assurance_v1.json"

keep_open() { echo; echo "Git Bash duoc giu mo."; exec bash; }
fail() { echo "FAILED: $*" >&2; keep_open; }
...
[[ "$actual_store_sha" == "$STORE_SHA256" ]] || fail "SQLite hash da thay doi: $actual_store_sha"

echo
echo "===== KIEM TRA CODE V35.1 ====="
python -m py_compile \
    ...
    src/he_thong_dinh_luong/exact_cash_ledger_readiness_v35_1.py \
    src/he_thong_dinh_luong/exact_cash_ledger_readiness_v35_1_safe_runner.py \
    tests/test_exact_cash_ledger_readiness_v35.py \
    tests/test_exact_cash_ledger_readiness_v35_1.py \
    || fail "py_compile V35.1 that bai"
python -m unittest \
    tests.test_exact_cash_ledger_readiness_v35 \
    tests.test_exact_cash_ledger_readiness_v35_1 -v \
    || fail "unit test V35.1 that bai"

RUN_ID="$(date +%Y%m%d-%H%M%S)"
OUTPUT_POSIX="$PWD/artifacts/exact-cash-ledger-readiness-v35-1-$RUN_ID"
...
if [[ -f "$DATA_ASSURANCE_POSIX" ]]; then
    echo "DATA_ASSURANCE_FOUND=$DATA_ASSURANCE_POSIX"
    ARGS+=(--data-assurance-report "$(cygpath -w "$DATA_ASSURANCE_POSIX")")
else
    echo "DATA_ASSURANCE_MISSING=$DATA_ASSURANCE_POSIX"
fi

echo
echo "===== CHAY V35.1 EXACT-LEDGER READINESS AUDIT ====="
echo "V34_ARTIFACT=$V34_POSIX"
echo "SQLITE_STORE=$STORE_POSIX"
echo "OUTPUT_DIR=$OUTPUT_POSIX"
echo "PRICE_BASIS_REQUIRES_DATA_ASSURANCE=true"
python -m he_thong_dinh_luong.exact_cash_ledger_readiness_v35_1_safe_runner "${ARGS[@]}"
STATUS=$?
...
import sys
out = Path(sys.argv[1])
failure = out / "run_failure_v35_1.json"
report_path = out / "exact_cash_ledger_readiness_v35_1.json"
if failure.is_file():
