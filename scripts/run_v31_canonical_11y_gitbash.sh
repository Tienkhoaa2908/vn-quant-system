...
EXPECTED_BARS=299466
EXPECTED_FIRST_DAY="2015-06-29"
EXPECTED_LAST_DAY="2026-07-31"
EXPECTED_DISTINCT_DAYS=2775
EXPECTED_DISTINCT_SYMBOLS=122
...
OUTPUT_DIR_POSIX="$PWD/artifacts/all-history-protocol-v31-canonical-11y-$RUN_ID"
mkdir -p "$OUTPUT_DIR_POSIX"
OUTPUT_DIR_WIN="$(cygpath -w "$OUTPUT_DIR_POSIX")"
PREFLIGHT_WIN="$OUTPUT_DIR_WIN\\canonical_11y_preflight_v31.json"
...
python - \
    ...
    "$EXPECTED_DISTINCT_SYMBOLS" \
    "$EXPECTED_CONFLICTS" <<'PY'
...
from he_thong_dinh_luong.all_history_protocol_v31 import _load_all_history_zip
...
(
    ...
    expected_first_day,
    expected_last_day,
    expected_distinct_days_text,
    expected_symbols_text,
    expected_conflicts_text,
) = sys.argv[1:]

store = Path(store_text).resolve()
input_zip = Path(input_zip_text).resolve()
report_path = Path(report_text).resolve()
preflight_path = Path(preflight_text).resolve()
...
expected_sqlite = {
    "row_count": int(expected_bars_text),
    "first_day": expected_first_day,
    "last_day": expected_last_day,
    "distinct_days": int(expected_distinct_days_text),
    "distinct_symbols": int(expected_symbols_text),
    "conflict_row_count": int(expected_conflicts_text),
}
...
rows, input_manifest, coverage = _load_all_history_zip(input_zip)
v22_report = json.loads(report_path.read_text(encoding="utf-8-sig"))

payload = {
    "status": "PASS",
    "schema_version": "canonical_11y_preflight_v31",
    "canonical_store": {
        "path": str(store),
    ...
    "canonical_v22_report": {
        ...
        "top_level_keys": sorted(v22_report) if isinstance(v22_report, dict) else [],
    },
...
python -m he_thong_dinh_luong.all_history_protocol_v31_safe_runner \
