...
BRANCH="agent/v74-macro-pit-ablation"
...
ART="$PWD/artifacts"
OUT="$ART/v74-macro-pit-$RUN_ID"
V68="$OUT/v68"; V70="$OUT/v70"; V74="$OUT/v74"
BUNDLE_DIR="$ART/v74-macro-pit-bundle-$RUN_ID"
BUNDLE="$ART/UPLOAD_THIS_v74_MACRO_PIT-$RUN_ID.zip"
FAIL_BUNDLE="$ART/UPLOAD_THIS_v74_MACRO_PIT_FAILURE-$RUN_ID.zip"
LOG="$ART/v74-macro-pit-$RUN_ID.log"
mkdir -p "$V68" "$V70" "$V74" "$BUNDLE_DIR"

run_all() (
  set -euo pipefail
  echo "===== V74 ONE-SHOT PUBLICATION-DATE PIT MACRO ABLATION ====="
  echo "BRANCH=$BRANCH"
  ...
  echo "MACRO_SOURCE=OFFICIAL_NSO_PUBLIC_ARCHIVES"
  echo "MACRO_SERIES=CPI_YOY,IIP_YOY"
  echo "PUBLICATION_DATE_PIT_ENFORCED=true"
  echo "FIRST_RELEASE_PER_REFERENCE_MONTH=true"
  echo "NETWORK_SCOPE=NSO_OFFICIAL_PUBLIC_MACRO_ONLY"
  ...
  echo "GATES=MACRO_IIP3_DECEL_SOFT50,MACRO_CPI3_ACCEL_SOFT50,MACRO_STAGFLATION3_SOFT50"
  echo "GATE_MAGNITUDE_THRESHOLD_TUNED=false"
  echo "SOFT_EXPOSURE=0.50"
  ...
  "$PY" -m py_compile \
    ...
    tests/test_macro_pit_ablation_v74.py
  "$PY" -m unittest tests.test_macro_pit_ablation_v74 -v
  echo
  ...
  echo "===== PHASE 2 V70: FROZEN-C3 DEEP BASELINE ====="
  ...
  echo "===== PHASE 3 V74: OFFICIAL NSO PUBLICATION-DATE PIT MACRO ====="
  "$PY" -m he_thong_dinh_luong.macro_pit_ablation_v74 \
    --v68-output "$(cygpath -w "$V68")" --v70-output "$(cygpath -w "$V70")" \
    --store "$(cygpath -w "$STORE")" --output-dir "$(cygpath -w "$V74")" \
    --signflip-samples 10000 --bootstrap-samples 5000
  echo

  echo "===== PROFIT FIRST ====="
  "$PY" - "$(cygpath -w "$V74/v74_backtest_summary.csv")" "$(cygpath -w "$V74/v74_candidate_inference.csv")" "$(cygpath -w "$V74/v74_2026_shadow.csv")" "$(cygpath -w "$V74/v74_report.json")" <<'PY'
import csv,json,sys
from pathlib import Path
with Path(sys.argv[1]).open("r",encoding="utf-8-sig",newline="") as f:
    for row in csv.DictReader(f):
        if row["cost_scenario"]=="BASE_DNSE" and row["settlement_mode"]=="IMMEDIATE" and float(row["initial_capital_vnd"])==1_000_000_000.0 and row["variant_id"] in {"BROAD_PROVISIONAL","GAP18_CLEAN"}:
            print("PNL",row["variant_id"],row["allocator"],row["policy_id"],"return="+row["total_return"],"benchmark="+row["benchmark_total_return"],"alpha="+row["total_alpha_arithmetic"],"cagr="+row["cagr"],"mdd="+row["max_drawdown_daily"])
with Path(sys.argv[2]).open("r",encoding="utf-8-sig",newline="") as f:
    for row in csv.DictReader(f):
        print("INFERENCE",row["variant_id"],row["allocator"],row["policy_id"],"mean_delta="+row["mean_monthly_return_delta"],"p="+row["signflip_two_sided_p"],"q="+row["bh_fdr_q"],"ci_low="+row["bootstrap_ci025"],"watch="+row["diagnostic_watchlist_gate_passed"])
with Path(sys.argv[3]).open("r",encoding="utf-8-sig",newline="") as f:
    for row in csv.DictReader(f):
        if row["variant_id"] in {"BROAD_PROVISIONAL","GAP18_CLEAN"}:
            print("Y2026",row["variant_id"],row["allocator"],row["policy_id"],"return="+row["strategy_return"],"benchmark="+row["benchmark_return"],"delta="+row["policy_minus_base_2026_return"],"april_delta="+row["april_2026_policy_minus_base"],"used_for_selection="+row["used_for_selection"])
report=json.loads(Path(sys.argv[4]).read_text(encoding="utf-8"))
print("MACRO_RELEASE_COUNT="+json.dumps(report["macro_release_count"],sort_keys=True))
