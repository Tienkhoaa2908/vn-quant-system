...
RUNNER="${V81_INNER_RUNNER:-scripts/run_v81_frozen_tactical_historical_audit_gitbash.sh}"
[[ -f "$RUNNER" ]] || fail "khong tim thay inner runner: $RUNNER"

mapfile -t DIRTY_TRACKED < <(git diff --name-only --)
if [[ "${#DIRTY_TRACKED[@]}" -eq 0 ]]; then
  echo "V81_WORKTREE_MODE=CLEAN_DIRECT"
  exec bash "$RUNNER"
fi
...
PATCH="$(mktemp "${TMPDIR:-/tmp}/v81-v78-web-state.XXXXXX.patch")"
git diff --binary -- "$WEBAPP" "$INDEX" > "$PATCH"
[[ -s "$PATCH" ]] || fail "dirty web state nhung patch rong"
PATCH_SHA="$(sha256sum "$PATCH" | awk '{print $1}')"
echo "V81_WORKTREE_MODE=PRESERVE_APPROVED_V78_WEB"
