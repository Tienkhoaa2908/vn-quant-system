...
echo
echo "===== KIEM TRA CODE V32.1 ====="

python -m py_compile \
    src/he_thong_dinh_luong/portfolio_ablation_v32.py \
    src/he_thong_dinh_luong/portfolio_ablation_v32_safe_runner.py \
    src/he_thong_dinh_luong/portfolio_ablation_v32_1.py \
    src/he_thong_dinh_luong/portfolio_ablation_v32_1_safe_runner.py \
    tests/test_portfolio_ablation_v32.py \
    tests/test_portfolio_ablation_v32_1.py \
    || fail "py_compile V32 that bai"

python -m unittest \
    tests.test_portfolio_ablation_v32 \
    tests.test_portfolio_ablation_v32_1 \
    -v \
    || fail "unit test V32.1 that bai"

V31_WIN="$(cygpath -w "$V31_POSIX")"
V22_WIN="$(cygpath -w "$V22_POSIX")"
RUN_ID="$(date +%Y%m%d-%H%M%S)"
OUTPUT_POSIX="$PWD/artifacts/portfolio-ablation-v32-1-canonical-11y-$RUN_ID"
OUTPUT_WIN="$(cygpath -w "$OUTPUT_POSIX")"

echo
echo "===== CHAY V32.1 FULL-HORIZON PORTFOLIO ABLATION ====="
echo "OUTPUT_DIR=$OUTPUT_POSIX"

python -m he_thong_dinh_luong.portfolio_ablation_v32_1_safe_runner \
...
if report_path.is_file():
    ...
    print("REQUESTED_BREADTHS=", report.get("requested_breadths"))
    print(
        "EVALUATED_FULL_HORIZON_BREADTHS=",
        report.get("evaluated_full_horizon_breadths"),
    )
    print(
        "INFEASIBLE_FULL_HORIZON_BREADTHS=",
        report.get("infeasible_full_horizon_breadths"),
    )
    print("FULL_HORIZON_PRESERVED=", report.get("full_horizon_preserved"))
    print(
...
feasibility_path = output / "breadth_feasibility_v32.csv"
if feasibility_path.is_file():
    with feasibility_path.open(
        "r", encoding="utf-8-sig", newline=""
    ) as stream:
        feasibility = list(csv.DictReader(stream))
    print()
    print("===== BREADTH FEASIBILITY =====")
    for row in feasibility:
        print(
            "BREADTH=", row.get("breadth"),
            "STATUS=", row.get("status"),
            "INSUFFICIENT_MONTHS=", row.get("insufficient_month_count"),
            "FIRST=", row.get("first_insufficient_month"),
            "COUNT=", row.get("first_insufficient_symbol_count"),
        )
