...
BRANCH="agent/v62-exposure-normalized-preview-bridge-study"
...
ART="$PWD/artifacts"
STUDY_ZIP="$ART/v62-exposure-bridge-$RUN_ID.zip"
BUNDLE="$ART/UPLOAD_THIS_v62_EXPOSURE_BRIDGE-$RUN_ID.zip"
FAIL_BUNDLE="$ART/UPLOAD_THIS_v62_EXPOSURE_BRIDGE_FAILURE-$RUN_ID.zip"
LOG="$ART/v62-exposure-bridge-$RUN_ID.log"
BUNDLE_DIR="$ART/v62-bundle-$RUN_ID"
mkdir -p "$ART" "$BUNDLE_DIR"

run_study(){
  echo "===== V62 EXPOSURE-NORMALIZED C3 PREVIEW-BRIDGE STUDY ====="
  echo "BRANCH=$BRANCH"
  echo "HEAD=$(git rev-parse HEAD)"
  echo "ANALYSIS_END=2026-07-31"
  echo "AUGUST_2026_VPI_SELECTION=false"
  echo "CAUSALITY=PREVIOUS_WEEK_CLOSE_TO_LATER_OPEN"
  echo "RECYCLE_REALIZED_CASH=true"
  echo "RISK_ON_STOCK_TARGET=1.0"
  echo "RISK_OFF_STOCK_TARGET=0.5"
  echo "BRIDGE_NAV_SIZING=true"
  echo "MATURE_RISK_WINDOW=week>=13_and_positions>=5"
  echo "FORMER_V60_HOLDOUT_ALREADY_CONSUMED=true"
  echo "LIVE_MODEL_CHANGE=false"
  echo
  echo "===== COMPILE + PURE TESTS ====="
  "$PY" -m py_compile src/he_thong_dinh_luong/c3_exposure_bridge_v62.py tests/test_c3_exposure_bridge_v62.py
  "$PY" -m unittest tests.test_c3_exposure_bridge_v62 -v
  echo
  echo "===== RUN LOCAL ARCHIVE STUDY ====="
  "$PY" -m he_thong_dinh_luong.c3_exposure_bridge_v62 \
